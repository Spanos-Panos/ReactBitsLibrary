...
const FIXED_CAM_Z = 2.0;
const CANVAS_FOV = 50;
// geoScale so that modelScale=100 fills 100% of canvas height at FIXED_CAM_Z
const scaleForPercent = (p: number) => (p / 100) * 2 * FIXED_CAM_Z * Math.tan(deg2rad(CANVAS_FOV / 2));
...
const Loader: FC<{ placeholderSrc?: string }> = ({ placeholderSrc }) => {
  ...
  return (
    <Html center>
      {placeholderSrc ? (
        <img src={placeholderSrc} alt="Loading..." width={128} height={128} className="blur-lg rounded-lg" />
      ) : (
...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  const ext = useMemo(() => url.split('.').pop()!.toLowerCase().split('?')[0], [url]);
  ...
  useLayoutEffect(() => {
    if (!content) return;
    inner.current.scale.setScalar(baseScale.current * geoScale);
    invalidate();
  }, [geoScale]);
  ...
  useFrame((_, dt) => {
    ...
    ndc.x += xOff / 100 + cPar.current.x;
    ndc.y += yOff / 100 + cPar.current.y;
...
const ModelViewer: FC<ViewerProps> = ({
  ...
  const camZ = FIXED_CAM_Z;
  const geoScale = scaleForPercent(modelScale);
  const shadowY = -(geoScale / 2);
  ...
  return (
    ...
    >
      ...
      <Canvas
        ...
        camera={{ fov: CANVAS_FOV, position: [0, 0, camZ], near: 0.01, far: 1000 }}
      ...
      >
        ...
        <ContactShadows ref={contactRef as any} position={[0, shadowY, 0]} opacity={0.35} scale={geoScale * 10} blur={2} />
