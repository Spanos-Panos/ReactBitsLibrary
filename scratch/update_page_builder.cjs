...
// 1. Fix withContentText
content = content.replace(/function withContentText\(compName, jsx, text\) \{[\s\S]*?\}/, 
...
// 2. Fix buildShowcaseSection
content = content.replace(/function buildShowcaseSection\(componentNames, aesthetic, layout\) \{[\s\S]*?\}/, 
...
// 3. Stricter Page Assignment (Services shouldn't have ContactForm)
// I'll update findComponentForSection to be more picky if possible.
// Or just update the section builders.

fs.writeFileSync(path, content);
console.log('Successfully updated page-builder.cjs via script');
